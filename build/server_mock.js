...
var mockDir = path.resolve(__dirname, config.mock.path);
...
function routerDefined (config) {
  ...
  if (method === 'GET') {
    ...
    server.put(config.api, function (req, res) {
      config.response(req, res);
      renderLog(req, res);
    });
  } else {
...
server.listen(config.mock.port, function (argument) {
  console.log('mockjson server: ' + color.red('http://localhost:' + config.mock.port));
